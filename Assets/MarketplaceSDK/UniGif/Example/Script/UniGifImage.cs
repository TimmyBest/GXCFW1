...
public class UniGifImage : MonoBehaviour
{
    ...
    // Activity Indicator
    [SerializeField]
    private RawImage activityIndicator;
        ...
        switch (nowState)
        {
            ...
            case State.Loading:
                if (m_rotateOnLoading)
                {
                    ...
                    activityIndicator.transform.Rotate(0f, 0f, 30f * Time.deltaTime, Space.Self);
                }
                break;
        ...
        using (WWW www = new WWW(path))
        {
            ...
            yield return StartCoroutine(UniGif.GetTextureListCoroutine(www.bytes, (gifTexList, loopCount, width, height) =>
            {
                if (gifTexList != null)
                {
                    ...
                    if (autoPlay)
                    {
                        m_rawImage.transform.localScale = new Vector3(1,1,1);
